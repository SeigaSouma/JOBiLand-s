...
void UpdateSuperSubEffect(int nCntEffect);
...
const char *c_apFilenameEffect[] =		//ファイル読み込み
{
	...
	"data\\TEXTURE\\thunder_01.tga",
	"data\\TEXTURE\\thunder_02.tga",
};
...
void InitEffect(void)
{
	...
	for (int nCntEffect =0; nCntEffect < MAX_EFFECT; nCntEffect++)
	{
		...
		g_aEffect[nCntEffect].mtxWorld = {};	//ワールドマトリックス
		g_aEffect[nCntEffect].col = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);		//色
		g_aEffect[nCntEffect].fRadius = 0.0f;		//半径
		g_aEffect[nCntEffect].fMaxRadius = 0.0f;	//最大半径
		g_aEffect[nCntEffect].nLife = EFFECT_LIFE;			//寿命
		g_aEffect[nCntEffect].nMaxLife = EFFECT_LIFE;		//最大寿命(固定)
		g_aEffect[nCntEffect].moveType = 0;		//移動の種類
		g_aEffect[nCntEffect].nType = 0;			//種類
		g_aEffect[nCntEffect].bUse = false;			//使用しているかどうか
		g_aEffect[nCntEffect].bAddAlpha = false;		//加算合成の判定
	}
	g_EffectNum = 0;
	g_bEffectUse = true;
	...
	for (int nCntEffect = 0; nCntEffect < MAX_EFFECT; nCntEffect++)
	{
		...
		{//エフェクトが使用されていたら
			...
			{//エフェクトを小さくしていく
				...
				if (g_aEffect[nCntEffect].nType == EFFECTTYPE_NORMAL || g_aEffect[nCntEffect].nType == EFFECTTYPE_SMOKE ||
					g_aEffect[nCntEffect].nType == EFFECTTYPE_SMOKEBLACK || g_aEffect[nCntEffect].nType == EFFECTTYPE_BLACK)
				{
					//縮小処理
					UpdateSubEffect(nCntEffect);
				}
			}
			else if (g_aEffect[nCntEffect].moveType == MOVEEFFECT_SUPERSUB)
			{//エフェクトを小さくしていく

				UpdateSuperSubEffect(nCntEffect);
			}
			else
			{//エフェクトを大きくしていく
...
void UpdateSubEffect(int nCntEffect)
{
	...
	//g_aEffect[nCntEffect].fRadius = g_aEffect[nCntEffect].fMaxRadius * (float)g_aEffect[nCntEffect].nLife / (float)g_aEffect[nCntEffect].nMaxLife;
...
void SetEffect(D3DXVECTOR3 pos, D3DXCOLOR col, float fRadius, int nLife, D3DXVECTOR3 move, int moveType, int nType)
{
	...
	for (int nCntEffect = 0; nCntEffect < MAX_EFFECT; nCntEffect++)
	{
		...
		{//エフェクトが使用されていなかったら
			...
			g_aEffect[nCntEffect].fMaxRadius = fRadius;
			...
			else if (g_aEffect[nCntEffect].nType == EFFECTTYPE_HART)
			{//ハートのエフェクト

				g_aEffect[nCntEffect].bAddAlpha = true;
			}
			else if (g_aEffect[nCntEffect].nType == EFFECTTYPE_JUJI)
			{//十字のエフェクト
