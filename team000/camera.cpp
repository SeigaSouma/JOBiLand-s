...
void InitTitleCamera(void)
{
	for (int nCntCamera = 0; nCntCamera < CAMERATYPE_MAX; nCntCamera++)
	{
		...
		if (nCntCamera == CAMERATYPE_SELECT)
		{
			...
			g_aCamera[nCntCamera].posV = D3DXVECTOR3(0.0f, 0.0f, g_aCamera[nCntCamera].posR.z + -600.0f);	//視点
			g_aCamera[nCntCamera].vecU = D3DXVECTOR3(0.0f, 1.0f, 0.0f);		//上方向ベクトル
			g_aCamera[nCntCamera].move = D3DXVECTOR3(0.0f, 0.0f, 0.0f);		//移動量
			g_aCamera[nCntCamera].nState = CAMERASTATE_NONE;				//状態
			g_aCamera[nCntCamera].fDistance = 1500.0f;	//距離
			...
			g_aCamera[nCntCamera].viewport.Width = SCREEN_WIDTH/* / 2*/;	//描画する画面の幅
			g_aCamera[nCntCamera].viewport.Height = SCREEN_HEIGHT;	//描画する画面の高さ
			g_aCamera[nCntCamera].viewport.X = 0;	//描画する画面の左上X座標
			g_aCamera[nCntCamera].viewport.Y = 0;	//描画する画面の左上Y座標
		}
...
void InitGameCamera(void)
{
	...
	for (int nCntCamera = 0; nCntCamera < CAMERATYPE_MAX; nCntCamera++)
	{
		...
		case CAMERATYPE_SIGN:
			g_aCamera[nCntCamera].viewport.Width = 250;	//描画する画面の幅
			g_aCamera[nCntCamera].viewport.Height = 250;	//描画する画面の高さ
			g_aCamera[nCntCamera].viewport.X = SCREEN_WIDTH - g_aCamera[nCntCamera].viewport.Width;	//描画する画面の左上X座標
			g_aCamera[nCntCamera].viewport.Y = SCREEN_HEIGHT - g_aCamera[nCntCamera].viewport.Height; //描画する画面の左上Y座標
			g_aCamera[nCntCamera].posR = D3DXVECTOR3(0.0f, 0.0f, 0.0f);		//注視点
			g_aCamera[nCntCamera].posV = D3DXVECTOR3(0.0f, 1000.0f, -100.0f);	//視点
			g_aCamera[nCntCamera].fDistance = 90.0f;
			break;

		case CAMERATYPE_MAP:
			g_aCamera[nCntCamera].viewport.Width = 200;				//描画する画面の幅
			g_aCamera[nCntCamera].viewport.Height = 250;			//描画する画面の高さ
			g_aCamera[nCntCamera].viewport.X = 20;	//描画する画面の左上X座標
			g_aCamera[nCntCamera].viewport.Y = SCREEN_HEIGHT - g_aCamera[nCntCamera].viewport.Height - 17; //描画する画面の左上Y座標
			g_aCamera[nCntCamera].posR = D3DXVECTOR3(0.0f, 0.0f, 0.0f);		//注視点
			g_aCamera[nCntCamera].posV = D3DXVECTOR3(0.0f, 3000.0f, -500.0f);	//視点
			g_aCamera[nCntCamera].rot = D3DXVECTOR3(0.0f, 0.0f, -1.5f);	//向き
