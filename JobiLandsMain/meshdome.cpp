...
#define WIDTH		(16)
#define HEIGHT		(8)
#define DOME_COL	(0.2f)		//メッシュドームの色
...
void InitMeshDome(void)
{
	...
	D3DXCreateTextureFromFile(pDevice,
		"data\\TEXTURE\\sky101.jpg",
		&g_pTextureMeshDome);

	//各要素初期化
	g_aMeshDome.pos = D3DXVECTOR3(0.0f, 0.0f, 0.0f);		//位置
	g_aMeshDome.rot = D3DXVECTOR3(0.0f, 0.0f, 0.0f);		//向き
	g_aMeshDome.col = D3DXCOLOR(DOME_COL, DOME_COL, DOME_COL, 1.0f);	//色

	//ワールドマトリックスの初期化
	D3DXMatrixIdentity(&g_aMeshDome.mtxWorld);
	g_aMeshDome.nNumIndex = (HEIGHT * ((WIDTH + 1) * 2)) + (2 * (HEIGHT - 1));			//インデックス数
...
void UninitMeshDome(void)
{
	...
	if (g_pIdxBuffMeshDomeLid != NULL)
	{
		g_pIdxBuffMeshDomeLid->Release();
		g_pIdxBuffMeshDomeLid = NULL;
	}
...
{
	//col.r += ((1.0f - 初期値) / GAME_TIME)

	if (GetMode() == MODE_GAME && *GetGameState() != GAMESTATE_START)
	{//ゲームの時

		//色変える
		g_aMeshDome.col.r += ((1.0f - DOME_COL) / (float)GAMETIME);
		g_aMeshDome.col.g += ((1.0f - DOME_COL) / (float)GAMETIME);
		g_aMeshDome.col.b += ((1.0f - DOME_COL) / (float)GAMETIME);
		...
		{//縦の頂点数分繰り返す
			...
			{//横の頂点数分繰り返す
				...
				if (nCntHeight < HEIGHT - 1)
				{
					pVtx[0].col = D3DXCOLOR(g_aMeshDome.col.r, g_aMeshDome.col.g, g_aMeshDome.col.b, 1.0f);
				}
				else
				{
					//頂点カラーの設定
